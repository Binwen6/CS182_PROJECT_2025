...
def train_step(model, xs, ys, optimizer, loss_func, grad_clip=None):
    optimizer.zero_grad()
    output = model(xs, ys)
...
def train(model, args):
    ...
    for i in pbar:
        ...
        loss, output = train_step(model, xs.to(device), ys.to(device), optimizer, loss_func, grad_clip=args.training.get("grad_clip", None))
...
if __name__ == "__main__":
    ...
    assert args.model.family in ["gpt2", "lstm", "mamba", "gla"]
