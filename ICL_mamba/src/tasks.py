...
class GaussianKernelRegression(Task):
    def __init__(
        ...
        if isinstance(noise_std, str) and noise_std.lower() == "auto":
            self.noise_std = "auto"
        else:
            self.noise_std = float(noise_std)

        if pool_dict is None and seeds is None:
            self.centers_b = torch.randn(batch_size, n_centers, n_dims)
            self.beta_b    = torch.randn(batch_size, n_centers, 1)
        elif pool_dict is not None:
            idx = torch.randperm(pool_dict["centers"].shape[0])[:batch_size]
            self.centers_b = pool_dict["centers"][idx]
            self.beta_b    = pool_dict["beta"][idx].unsqueeze(-1)
        else:
            gen = [torch.Generator().manual_seed(s) for s in seeds]
            self.centers_b = torch.stack([
                torch.randn(n_centers, n_dims, generator=g) for g in gen
            ])
            self.beta_b = torch.stack([
                torch.randn(n_centers, generator=g).unsqueeze(-1) for g in gen
            ])
    ...
    def evaluate(self, xs_b):
        ...
        d2 = ((xs - centers)**2).sum(-1)  # [B, N, C]
        features = torch.exp(-d2 / (2 * self.bandwidth ** 2))  # [B, N, C]

        ys_b = self.scale * (features @ self.beta_b).squeeze(-1)  # [B, N]
        # ys_b = ys_b * math.sqrt(self.n_dims) / ys_b.std()   # 让 Var(y) ≈ n_dims
        ys_b = (ys_b - ys_b.mean(dim=1, keepdim=True)) / (ys_b.std(dim=1, keepdim=True) + 1e-8)
        if self.noise_std == "auto":
            batch_std = ys_b.std()
            ys_b = ys_b + 0.1 * batch_std * torch.randn_like(ys_b)
        elif self.noise_std > 0:
            ys_b = ys_b + self.noise_std * torch.randn_like(ys_b)
        return ys_b

    @staticmethod
    def generate_pool_dict(n_dims, num_tasks, n_centers=20, **kwargs):
        return {
            "centers": torch.randn(num_tasks, n_centers, n_dims),
            "beta":    torch.randn(num_tasks, n_centers),
        }
...
class NonlinearDynamics(Task):
    def __init__(
        ...
        super(NonlinearDynamics, self).__init__(n_dims, batch_size, pool_dict, seeds)
        self.scale = scale
        self.noise_std = noise_std
    ...
    def poly(self, xs_b):
        ...
        ys = pred
        ys = (ys - ys.mean(dim=1, keepdim=True)) / (ys.std(dim=1, keepdim=True) + 1e-6)
        return ys
