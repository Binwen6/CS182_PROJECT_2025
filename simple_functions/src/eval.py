...
def get_model_from_run(run_path, step=-1, only_conf=False):
    ...
    if step == -1:
        ...
        state = torch.load(state_path, map_location='cuda:0' if torch.cuda.is_available() else 'cpu')
...
def eval_batch(model, task_sampler, xs, xs_p=None):
    task = task_sampler()
    if torch.cuda.is_available() and model.name.split("_")[0] in ["gpt2", "lstm", "mamba"]:
        device = "cuda"
    else:
        device = "cpu"
...
def build_evals(conf, n_test_points=None):
    ...
    data_name = "gaussian"  # conf.training.data

    # Hack to make test work for the skewed case
    # if task_name == "skewed_linear_regression":
    ...
    base_kwargs = {
        ...
        # "task_sampler_kwargs": {"exp": 1}
    }

    evaluation_kwargs = {}

    # 为这几类任务添加 standard + 3 种 held-out 评估策略
    if task_name in [
...
def get_run_metrics(
    run_path, step=-1, cache=True, skip_model_load=False, skip_baselines=False, n_test_points=None,
    no_recompute=False,
):
    if skip_model_load:
        ...
        model = model.to('cuda').eval() if torch.cuda.is_available() else model.eval()
        all_models = [model]
        if not skip_baselines:
            all_models += models.get_relevant_baselines(conf.training.task)

    evaluation_kwargs = build_evals(conf, n_test_points=n_test_points)
...
if __name__ == "__main__":
    run_dir = sys.argv[1]
    run_id_selected = sys.argv[2] if len(sys.argv) > 2 else None
    n_test_points = int(sys.argv[3]) if len(sys.argv) > 3 else 100 #120
    for task in os.listdir(run_dir):
        task_dir = os.path.join(run_dir, task)
        print(f"Evaluating {task=}, with {n_test_points=}")
        for run_id in tqdm(os.listdir(task_dir)):
            if run_id_selected is None or run_id == run_id_selected:
                run_path = os.path.join(run_dir, task, run_id)
                print(f"  Evaluating {run_path=}")
                metrics = get_run_metrics(run_path, n_test_points=n_test_points)
